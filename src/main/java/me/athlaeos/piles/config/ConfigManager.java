package me.athlaeos.piles.config;
...
public class ConfigManager {
    ...
    public static YamlConfiguration getDefault(String name) {
        try {
            ...
            Piles.logSevere("Default config " + name + " was called, but doesn't exist in plugin!");
            return null;
        }
    ...
    public static YamlConfiguration fetchDefaultConfiguration(String name) throws FileNotFoundException {
        InputStream customClassStream = Piles.getInstance().getClass().getResourceAsStream("/" + name);
